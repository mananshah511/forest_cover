...
from forest.entity.artifact_entity import ModelEvulationArtifact,ModelTrainerArtifact,DataTransformArtifact
from forest.entity.config_entity import ModelEvulationConfig
import pandas as pd
import numpy as np
from forest.entity.model_factory import get_evulated_classification_model
from forest.constant import BEST_MODEL_KEY,HISTORY_KEY,MODEL_PATH_KEY
from forest.util.util import read_yaml,write_yaml_file,load_object
...
class ModelEvulation:
    ...
    def get_best_model(self,clutser_number):
        try:
            logging.info(f"get best model function started")
            model_evulation_path = self.model_evulation_config.evulation_file_path

            logging.info(f"model evulation path : {model_evulation_path}")

            model = None
            CLUSTER_NUMBER = 'cluster'+str(clutser_number)
    ...
    def get_updated_evulation_report(self,cluster_number,model_evulation_artifact:ModelEvulationArtifact):
        try:
            ...
            logging.info(f"writting successful")
        except Exception as e:
            raise ForestException(sys,e) from e
    ...
    def intiate_model_evulation(self)->ModelEvulationArtifact:
        try:
            ...
            for cluster_number in range(len(train_files)):
                ...
                model = self.get_best_model(clutser_number=cluster_number)

                if model is None:
                    logging.info(f"no model found hence accepting the model")
