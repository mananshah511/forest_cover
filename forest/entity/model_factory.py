...
def get_evulated_classification_model(model_list:list,X_train:np.ndarray,y_train:np.ndarray,X_test:np.ndarray,
    ...
    try:
        ...
        for model in model_list:
            ...
            if model_accuracy>=base_accuracy and diff_train_test_accuracy<0.10:
...
class ModelFactory:
    ...
    def __init__(self,config_path:str=None) -> None:
        try:
            self.config:dict = ModelFactory.read_params(file_path=config_path)
            ...
            self.grid_search_cv_class_property_data:dict = dict(self.config[GRID_SEARCH_KEY][PARAM_KEY])
    ...
    def read_params(file_path:str)->dict:
        try:
            with open(file_path) as yaml_files:
    ...
    @staticmethod
    def update_property_class(insta_ref:object,property_data:dict):
        try:
    ...
    def get_intlized_model_list(self):
        try:
            logging.info(f"intlized model list function started")
            intlized_model_list = []

            for model_serial_numeber in self.model_intial_config.keys():

                model_config = self.model_intial_config[model_serial_numeber]
                ...
                if PARAM_KEY in model_config:
                    ...
                    model = ModelFactory.update_property_class(insta_ref=model,property_data=model_object_property_data)
                ...
                model_config = InitlizedModelDetails(model=model,
                                                     model_serial_number=model_serial_numeber,
    ...
    def initite_best_parameter_search_for_initlized_model(self,intlized_model_details:InitlizedModelDetails,
        ...
        try:
            ...
            grid_search_cv = ModelFactory.update_property_class(insta_ref=grid_search_cv,
                                                                property_data=self.grid_search_cv_class_property_data)
    ...
    def get_best_model_from_grid_serached_best_model_list(grid_searched_best_model_list:List[GridSearchedBestModel],
        ...
        try:
            logging.info(f"get best model from grid seaarched best models")
            best_model = None
    ...
    def get_best_model(self,X,y,base_accuracy:float=0.6):
        try:
            ...
            logging.info(f"Best individual models with parameter is : {grid_searched_best_model_list}")
